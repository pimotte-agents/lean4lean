...
theorem TrProj.weak' (W : Ctx.Lift' n Γ Γ')
    (H : TrProj Γ s i e e') : TrProj Γ' s i (e.lift' n) (e'.lift' n) := by
  unfold TrProj at H ⊢
  obtain ⟨C, numParams, hHead, hIdx⟩ := H
  refine ⟨C, numParams, ?_, ?_⟩
...
theorem TrProj.instN (W : Ctx.InstN Γ₀ e₀ A₀ k Γ₁ Γ)
    (H : TrProj Γ₁ s i e e') : TrProj Γ s i (e.inst e₀ k) (e'.inst e₀ k) := by
  unfold TrProj at H ⊢
  obtain ⟨C, numParams, hHead, hIdx⟩ := H
  refine ⟨C, numParams, ?_, ?_⟩
...
theorem TrProj.instL (H : TrProj Γ s i e e') :
    TrProj (Γ.map (VExpr.instL ls)) s i (e.instL ls) (e'.instL ls) := by
  unfold TrProj at H ⊢
  obtain ⟨C, numParams, hHead, hIdx⟩ := H
  refine ⟨C, numParams, ?_, ?_⟩
